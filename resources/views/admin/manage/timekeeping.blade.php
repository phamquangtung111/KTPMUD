        ...
        <div>
            ...
            <table class="table table-striped custom-table mb-0">
                ...
                <script>
                    ...
                    @foreach($checkin as $val)
                        @if ($val->macheckin == $item->id)
                            <script>
                                var tmp = "{{$val->date}}".split('/')[0]
                                new_id[tmp] = "{{$val->macheckin}}"
                                date_checkin[tmp] ="{{$val->date}}"
                                first[tmp] = "{{$val->checkin_time}}"
                                last[tmp] = "{{$val->checkout_time == null ? 1 : 2}}"
                                check++
                            </script>
                        @endif
                    @endforeach
                <script>
                    chart_members.push(["{{$item->tennv}}", check])
                </script>
                <script>
